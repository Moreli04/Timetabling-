...
public:
	...
	int** MatrizDisciplinaDisciplina;
	MeuTimeslot *timeslots;
	char nome[30];
	int nDisciplinas;
	int nCurriculos;
...
//	int pesoHard;
	...
	int SaoDoMesmoCurriculoAntigo(Disciplina* d1, Disciplina* d2);
	...
	Linha* preencherMatriz(int ordem);
	void escreverNoArquivo(Linha*lin, char* url);
	void restaurarAulasNaoAlocadas();
	void ContarCurriculoEProfessorParaCadaDisc();
	int getCapacidadeSala(int numeroSequencial);
	void ordenar();
	virtual ~Problema();
};
